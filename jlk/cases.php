...
if(!$smarty->isCached('cases.html',$id)){
	$model=new LanmuModel();
	$model->table('about');
	...
	$model->id='cat_id';
	$cases_sub_result=$model->find($id);
	...
	$model->table('cases');
	$model->id='cat_id';
	$cases_result_content=$model->findAll($id);
	$smarty->assign('cases_result_content',$cases_result_content);
}
$smarty->display('cases.html',$id);
